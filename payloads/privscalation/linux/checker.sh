...
then
  echo "WP-CONFIG.PHP FOUND! " && cat $(find / -iname wp-config.php 2>/dev/null) | grep  -E "DB_NAME|DB_USER|DB_PASSWORD|DB_HOST"
else
...
echo "                                    "
touch ~/.bash_history 
cat /etc/passwd | cut -d ":" -f 6 | while read -r line; do echo "$line/.bash_history"; cat "$line/.bash_history"; echo ""; done                                                                                    # Clear Command History
echo    "                                          "
echo -e "\e[35m#----------------------------------#"
